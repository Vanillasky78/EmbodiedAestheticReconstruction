...
def procrustes_pose_score(kps_a: np.ndarray, kps_b: np.ndarray) -> float:
    ...
    A = kps_a.astype(np.float32)
    B = kps_b.astype(np.float32)
...
def _face_angles_from_landmarks(landmarks, w: int, h: int) -> Optional[Tuple[float,float,float]]:
    ids = [33, 263, 1, 152]  # left eye outer, right eye outer, nose tip, chin
    try:
...
# ---- Silhouette similarity (via selfie segmentation) ----
...
def silhouette_ssim(pil_img: Image.Image, pil_ref: Image.Image) -> float:
    m1 = person_mask(pil_img)
    m2 = person_mask(pil_ref)
    h, w = m1.shape[:2]
...
# ---- Color similarity (HSV hist correlation) ----
def color_corr(pil_img: Image.Image, pil_ref: Image.Image) -> float:
    ...
    sim = cv2.compareHist(h1, h2, cv2.HISTCMP_CORREL)  # [-1,1]
