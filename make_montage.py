...
Usage B (use existing CSV from batch_test.py):
    python make_montage.py --csv batch_results.csv --outdir montages_csv

Notes:
- Works on Windows/Linux/macOS.
- Requires: pillow, opencv-python, numpy, tqdm (and for retrieval mode: open_clip_torch, torch, faiss-cpu, mediapipe)
"""
...
def fit_into_box(img_bgr: np.ndarray, box_wh: Tuple[int,int], pad_color=(245,245,245)) -> np.ndarray:
    """Letterbox resize into box (width,height), keep aspect, pad with pad_color."""
    w, h = box_wh
...
def draw_text(pil_img: Image.Image, text: str, xy=(10,10), fill=(20,20,20), size=22):
    """Robust text drawing (fallback to default font)."""
    draw = ImageDraw.Draw(pil_img)
    try:
        # Windows: "arial.ttf" often available; else fallback
...
def make_montage(
    ...
    └────────────────┴─────────────────┘
    """
    # compute grid for matches (2 cols)
    cols = 2
    ...
    # left panel height matches grid height
    left_h = grid_h
    ...
    draw_text(pil_canvas, title, (pad, 10), size=28)
    draw_text(pil_canvas, info, (pad, 40), size=20)
    canvas = bgr_from_pil(pil_canvas)

    # paste input image (left)
    try:
        inp = read_bgr(input_path)
    except Exception:
        # try PIL open
        inp = cv2.cvtColor(np.array(Image.open(input_path).convert("RGB")), cv2.COLOR_RGB2BGR)

    inp_fit = fit_into_box(inp, (left_w, left_h))
    canvas[header_h+pad:header_h+pad+left_h, pad:pad+left_w] = inp_fit

    # draw left panel caption
    pil_canvas = pil_from_bgr(canvas)
    draw_text(pil_canvas, "Input", (pad+10, header_h+pad+10), size=20, fill=(70,70,70))
    canvas = bgr_from_pil(pil_canvas)

    # paste matches grid (right)
    x0 = pad*2 + left_w
    y0 = header_h + pad
    ...
    for idx, m in enumerate(matches):
        ...
        mg_fit = fit_into_box(mg, (cell_w, cell_h-42))  # reserve text bar
        canvas[gy:gy+cell_h-42, gx:gx+cell_w] = mg_fit

        # score bar
        bar = np.full((42, cell_w, 3), (250,250,250), dtype=np.uint8)
        ...
        draw_text(pil_bar, f"{fname} | {clip_s}{pose_s}{fused_s}", (8, 8), size=18, fill=(40,40,40))
        bar = bgr_from_pil(pil_bar)
        canvas[gy+cell_h-42:gy+cell_h, gx:gx+cell_w] = bar
...
def run_retrieval_and_export(folder: str, index_dir: str, outdir: str, topk: int, alpha: float):
    if not RETRIEVAL_AVAILABLE:
        raise RuntimeError("Retrieval modules not available. Run from project root and ensure processors/* importable.")
    ...
    for ipath in tqdm(list(iter_images(folder)), desc="Processing"):
        ...
        make_montage(ipath, items, outpath)
...
def export_from_csv(csv_path: str, outdir: str, topk: int):
    mapping = load_csv(csv_path)
    for ipath, items in tqdm(mapping.items(), desc="Rendering"):
        chosen = items[:topk]
        outname = os.path.splitext(os.path.basename(ipath))[0] + f"_top{topk}.jpg"
        outpath = os.path.join(outdir, outname)
        make_montage(ipath, chosen, outpath)
...
def main():
    ...
    if args.csv:
        export_from_csv(args.csv, args.outdir, args.topk)
    elif args.folder:
